...
for i,id in enumerate(input):
	input[i] = ''
	result = difflib.get_close_matches( id, input, 1, 0.95)
	input[i] = id
	if ( result ):
		print( i, ':' )
		print( '>', id )
		print( '<', result[0] )
		for j, a in enumerate(result[0]):
			if a != id[j]:
				print( 'differ with:', a, id[j] )
				match.append(id[0:j] + id[(j+1):])
				print( match )
				
				
				
#cvgywxqubnuaefmsldjdrpfzyi
#cvgywxqubnuaefmsl jdrpfzyi
#cvgywxqubnuaefms  jdrpfzyi	
#cvgywxqubnuaefms  jdrpfzyi
